...
def main(_):
	...
	with codecs.open(FLAGS.data_path + '/vocab','w','utf-8') as f:
		for data in vocab:
			f.write(data + '\n');
	...
	with sv.managed_session() as session:
		for i in range(config.max_max_epoch):
			...
			train_perplexity = run_epoch(session, m,id_to_word=vocab, eval_op=m.train_op,verbose=True)
			print("Epoch: %d Train Perplexity: %.3f" % (i + 1, train_perplexity))

		test_perplexity = run_epoch(session, mtest,id_to_word=vocab)
